...
target_dir="$build_dir/SOFACI_Release/"

platform="linux"
version=1.0.00
release=1
sc_name="gen_sofaCItest_package.sh"
...
function build_sofaCI()
{
    cd $build_dir/utils/ci_autotest/
    sh install_ci_testscript.sh $target_dir
    cd $build_dir
    return 0
}
...
function gen_packages()
{
    ...
    if [ "$1" == "linux" ]; then
        sh Generate_sofaCI_RPM.sh $2 $3 $4
        /bin/rm -rf $build_dir/packages/SOFACI-*.rpm
        /bin/mv SOFACI-*.rpm $build_dir/packages/
    else
        sh Generate_sofaCI_deb.sh $2 $3 $4
    fi

    cd ../DeployScript

    /bin/cp deploy_sofaCI.sh $build_dir/packages/
    /bin/cp undeploy_sofaCI.sh $build_dir/packages/
    cd $build_dir
}
