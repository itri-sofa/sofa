...
jLego.objectUI.ccmaTabPage=function(){
    var myID;
    var myClass;
    var myConsts;
    var randomID;
...
jLego.objectUI.ccmaTabPage.prototype.initialize=function(){
    this.myID=jLego.objectUI.id.ccmaTabPage;
    this.myClass=jLego.objectUI.cls.ccmaTabPage;
    this.myConsts=jLego.objectUI.constants.ccmaTabPage;
...
jLego.objectUI.ccmaTabPage.prototype.add=function(target, option){
    //check option
    ...
    this.tabTitle = 
            jLego.basicUI.addDiv(this.tabFrame, {id: jLego.func.getRandomString(), class: this.myClass.TABPAGE_TAB_TITLE});
...
jLego.objectUI.ccmaTabPage.prototype.addTabElement=function(name, callbackObject){
    ...
    if(tabIndex==0){
        ...
        //else $(this.tabList[tabIndex]).css('right', '0px');
        $(this.tabList[tabIndex]).css('right', '10px');
    }
    else{
        $(this.tabList[tabIndex]).css('right', '10px');
        ...
        for(var i=tabIndex-1; i>=0; i--){
            var marginRight = $(this.tabList[i+1]).width() + parseInt($(this.tabList[i+1]).css('right'))  + parseInt($(this.tabList[i+1]).css('padding-left'))  + parseInt($(this.tabList[i+1]).css('padding-right'));
            $(this.tabList[i]).css('right', marginRight + 'px');
        }
    }
    ...
    $(this.tabList[tabIndex]).click(function(){
        ...
        if($(this).attr('class').toString().indexOf(parent.myClass.TABPAGE_TAB_OFF)>-1){
            ...
            if(parent.mode=='page'){
                ...
                //$(parent.tabTitle).text($(this).text());
            }
...
jLego.objectUI.ccmaTabPage.prototype.getTab=function(index){
    if(index >= this.tabList.length)    return null;
    else return this.tabList[index];
}

jLego.objectUI.ccmaTabPage.prototype.getTabPage=function(index){
    if(index >= this.tabPageList.length)    return null;
    else return this.tabPageList[index];
}

jLego.objectUI.ccmaTabPage.prototype.getPermanentPage=function(){
    return this.permanentPage;
}
jLego.objectUI.ccmaTabPage.prototype.getSelectedTab=function(){
    return this.selectTabIndex;
}

jLego.objectUI.ccmaTabPage.prototype.resize=function(){
...
jLego.objectUI.ccmaTabPage.prototype.resizeHandler=function(){
    this.resize();
}