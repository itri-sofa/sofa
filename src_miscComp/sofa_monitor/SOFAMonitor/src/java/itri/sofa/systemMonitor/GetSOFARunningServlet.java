 ...
 * @author A40385
 */
@WebServlet(name = "GetSOFARunningServlet", urlPatterns = {"/GetSOFARunningServlet"})
public class GetSOFARunningServlet extends HttpServlet{
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            JSONObject runningStatusData = sofaAPI.getSystemRunningStatus();
            System.out.println(runningStatusData.toString());
            if(runningStatusData == null){
                 returnJSON = new JSONObject();
                 returnJSON.put(Key.ERR_CODE, Status.ERR_CODE_FAIL);
            }
            else{
                returnJSON = runningStatusData;
                returnJSON.put(Key.ERR_CODE, Status.ERR_CODE_OK);
            }
            out.print(returnJSON.toString());
        } catch (JSONException ex) {
            Logger.getLogger(GetSOFARunningServlet.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            out.close();
        }  
