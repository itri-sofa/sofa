 ...
 * @author A40385
 */
@WebServlet(name = "SetFreeListThresholdServlet", urlPatterns = {"/SetFreeListThresholdServlet"})
public class SetFreeListThresholdServlet extends HttpServlet{
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String freeListThresholdString = request.getParameter("freeListThreshold");
        double freeListThreshold = Double.valueOf(freeListThresholdString);
        if(fakeDataModeString!=null && fakeDataModeString.toLowerCase().matches("true"))
            fakeDataMode = true;
        BasicAPI sofaAPI = new BasicAPI(fakeDataMode);
        JSONObject returnJSON;   
        try {
            JSONObject resultData = sofaAPI.setFreeListThreshold(freeListThreshold);
            ...
            Logger.getLogger(SetFreeListThresholdServlet.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            out.close();
        }  
