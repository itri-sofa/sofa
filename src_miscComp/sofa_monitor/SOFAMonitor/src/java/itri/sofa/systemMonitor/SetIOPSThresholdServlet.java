 ...
 * @author A40385
 */
@WebServlet(name = "SetIOPSThresholdServlet", urlPatterns = {"/SetIOPSThresholdServlet"})
public class SetIOPSThresholdServlet extends HttpServlet{
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String iopsThresholdString = request.getParameter("iopsThreshold");
        double iopsThreshold = Double.valueOf(iopsThresholdString);
        if(fakeDataModeString!=null && fakeDataModeString.toLowerCase().matches("true"))
            fakeDataMode = true;
        BasicAPI sofaAPI = new BasicAPI(fakeDataMode);
        JSONObject returnJSON;   
        try {
            JSONObject resultData = sofaAPI.setIOPSThreshold(iopsThreshold);
            ...
            Logger.getLogger(SetIOPSThresholdServlet.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            out.close();
        }  
