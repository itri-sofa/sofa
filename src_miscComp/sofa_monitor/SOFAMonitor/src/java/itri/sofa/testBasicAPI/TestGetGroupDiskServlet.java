 ...
 * @author A40385
 */
@WebServlet(name = "TestGetGroupDiskServlet", urlPatterns = {"/TestGetGroupDiskServlet"})
public class TestGetGroupDiskServlet extends HttpServlet{
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String groupID = request.getParameter(Key.GROUP_ID); 
        ...
        try {
            if(groupID==null){
                ...
                JSONObject diskSMARTData = sofaAPI.getGroupDisks(Integer.valueOf(groupID));
                returnJSON = diskSMARTData;
                returnJSON.put(Key.ERR_CODE, Status.ERR_CODE_OK);
            }
            out.print(returnJSON.toString());
        } catch (JSONException ex) {
            Logger.getLogger(TestGetGroupDiskServlet.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            out.close();
        }  
